...
std::unique_ptr<ClientApp> g_ClientApp;
...
DWORD WINAPI ClientThread(LPVOID lpParam)
{
    ...
    try {
        ...
        g_ClientApp = std::make_unique<ClientApp>();
        
        LOG_INFO(LOG_COMPONENT_CONSOLE, "Initializing network...");
        if (!g_ClientApp->Initialize()) {
            LOG_ERROR(LOG_COMPONENT_CONSOLE, "Failed to initialize client: " << g_ClientApp->GetLastError());
            UnloadDLL();
            return 1;
        }
        ...
        while (!exitRequested) {
            ...
            if (!g_ClientApp->IsRegistered()) {
                ...
                LOG_INFO(LOG_COMPONENT_CONSOLE, "Connecting to server and sending registration packet...");
                
                if (!g_ClientApp->RegisterWithServer()) {
                    LOG_ERROR(LOG_COMPONENT_CONSOLE, "Failed to register with server: " << g_ClientApp->GetLastError());
                    failedReconnectAttempts++;
                    
                    if (failedReconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
                        LOG_ERROR(LOG_COMPONENT_CONSOLE, "Failed to reconnect after " << MAX_RECONNECT_ATTEMPTS << " attempts.");
                        UnloadDLL();
                        return 1;
                    }
                    
                    LOG_INFO(LOG_COMPONENT_CONSOLE, "Will try again in 5 seconds (attempt " << failedReconnectAttempts << "/" << MAX_RECONNECT_ATTEMPTS << ")...");
                    Sleep(HEALTH_CHECK_INTERVAL);
                    continue;
                }
                
                LOG_INFO(LOG_COMPONENT_CONSOLE, "Registration successful!");
                LOG_INFO(LOG_COMPONENT_CONSOLE, "Connected to server. Heartbeats will be sent every " << HEALTH_CHECK_INTERVAL/1000 << " seconds.");
            ...
            if (currentTime - lastHealthCheckTime >= HEALTH_CHECK_INTERVAL) {
                ...
                if (!g_ClientApp->SendHealthCheck()) {
                    failedHealthChecks++;
                    LOG_WARN(LOG_COMPONENT_CONSOLE, "Health check failed (" << failedHealthChecks << "/" << MAX_FAILED_HEALTH_CHECKS << ")");
                    
                    if (failedHealthChecks >= MAX_FAILED_HEALTH_CHECKS) {
                        LOG_WARN(LOG_COMPONENT_CONSOLE, "Multiple health checks failed. Attempting to reconnect...");
