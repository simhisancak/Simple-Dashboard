...
namespace Network {
    ...
    bool NetworkClient::Connect(const std::string& ip, uint16_t port) {
        ...
        // Increase socket buffer sizes to handle larger packets
        int BufferSize = MAX_BUFFER_SIZE; // 1MB send buffer
        if (setsockopt(m_Socket, SOL_SOCKET, SO_SNDBUF, (char*)&BufferSize, sizeof(BufferSize))
            == SOCKET_ERROR) {
            LOG_WARN(LOG_COMPONENT_NETWORK,
                     "Failed to set send buffer size: " << WSAGetLastError());
        }
        if (setsockopt(m_Socket, SOL_SOCKET, SO_RCVBUF, (char*)&BufferSize, sizeof(BufferSize))
            == SOCKET_ERROR) {
            LOG_WARN(LOG_COMPONENT_NETWORK,
                     "Failed to set receive buffer size: " << WSAGetLastError());
        }
        ...
        while (m_Running) {
            ...
            if (result > 0) {
                int received = recv(m_Socket,
                                    ...
                                    static_cast<int>(MAX_BUFFER_SIZE - m_RecvBufferPos),
                                    0);
                ...
                while (m_RecvBufferPos - processedBytes >= sizeof(Packets::PacketHeader)) {
                    auto header = reinterpret_cast<Packets::PacketHeader*>(m_RecvBuffer.get()
                                                                           + processedBytes);
                    ...
                    case Packets::PacketType::MemoryRequest:
                        ...
                        break;
                    case Packets::PacketType::ItemDumpRequest:
                        packetSize = sizeof(Packets::ItemDumpRequestPacket);
                        break;
    ...
    bool NetworkClient::ProcessPacket(const char* data, size_t size) {
        ...
        try {
            ...
            case Packets::PacketType::MemoryRequest: {
                ...
                Packets::MemoryResponsePacket memoryResponse;
                memoryResponse.State = m_ClientApp->GetMemoryState();

                LOG_DEBUG(LOG_COMPONENT_NETWORK,
                          "Sending memory response - Size: " << sizeof(memoryResponse));
                return SendPacketInternal(&memoryResponse, sizeof(memoryResponse));
            }
            case Packets::PacketType::ItemDumpRequest: {
                if (size != sizeof(Packets::ItemDumpRequestPacket)) {
                    LOG_ERROR(LOG_COMPONENT_NETWORK, "Invalid ItemDumpRequest packet size");
                    return false;
                }

                auto itemDumpRequest = reinterpret_cast<const Packets::ItemDumpRequestPacket*>(
                    data);
                Packets::ItemDumpResponsePacket itemDumpResponse;
                itemDumpResponse.State = m_ClientApp->GetItemDumpState(itemDumpRequest->Filter);

                LOG_DEBUG(LOG_COMPONENT_NETWORK, "Processing item dump request");

                return SendPacketInternal(&itemDumpResponse, sizeof(itemDumpResponse));
            }
            case Packets::PacketType::SettingsResponse: {
                ...
                auto settingsPacket = reinterpret_cast<const Packets::SettingsResponsePacket*>(
                    data);
                return m_ClientApp->ProcessSettingsPacket(*settingsPacket);
            }
            default:
                LOG_ERROR(LOG_COMPONENT_NETWORK,
                          "Unknown packet type: " << static_cast<int>(header->Type));
                return false;
            }
    ...
    bool NetworkClient::SendPacketInternal(const void* data, size_t size) {
        ...
        while (totalSent < size) {
            int sent = send(m_Socket, buffer + totalSent, static_cast<int>(size - totalSent), 0);
    ...
    bool NetworkClient::SendItemDumpResponsePacket(const Packets::ItemDumpResponsePacket& packet) {
        std::lock_guard<std::mutex> lock(m_SendMutex);
        return SendPacketInternal(&packet, sizeof(packet));
    }
