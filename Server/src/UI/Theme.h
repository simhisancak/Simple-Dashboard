...
namespace UI {
    ...
    struct ThemeConfig {
        ...
        static void DrawSeparator(bool isVertical = false, float size = 0.9f) {
            ...
            if (!isVertical) {
                float width;
                if (size <= 0.0f) {
                    width = contentSize.x * 0.9f;
                } else if (size <= 1.0f) {
                    width = contentSize.x * size;
                } else {
                    width = contentSize.x - size;
                }
                ...
                float height;
                if (size <= 0.0f) {
                    height = contentSize.y * 0.9f;
                } else if (size <= 1.0f) {
                    height = contentSize.y * size;
                } else {
                    height = contentSize.y - size;
                }
    ...
    public:
        static void Apply() {
            ...
            const ImVec4 accent = ImVec4(0.28f, 0.56f, 0.90f, 1.00f);
            ...
            style.FramePadding = ImVec2(ThemeConfig::ButtonPadding, ThemeConfig::ButtonPadding);
