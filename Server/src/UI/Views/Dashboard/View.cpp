...
void DashboardView::Render() {
    ...
    if (ImGui::Begin("DashboardContainer", nullptr, window_flags)) {
        ...
        // Title bar - Full width, no padding
        RenderTitleBar("Dashboard", true);

        // Content area with padding only on the right side of sidebar
        if (ImGui::BeginChild("Content", ImVec2(0, 0), false)) {
            // Sidebar - No left padding, extends to bottom
            ...
            // Main Area - Extends to right and bottom edges
            ImGui::BeginChild("MainArea", ImVec2(0, 0), false);

            // Header
            ImGui::PushStyleColor(ImGuiCol_ChildBg, ThemeConfig::BgMid());
            ImGui::BeginChild("Header",
                              ImVec2(ImGui::GetContentRegionAvail().x, ThemeConfig::HeaderHeight),
                              false,
                              ImGuiWindowFlags_NoScrollbar);
            {
                m_Header->Render(m_MainContent->GetContents(), m_ActiveContent);

                // Separator'ı header'ın içine al ve en alta yerleştir
                ImGui::SetCursorPosY(ThemeConfig::HeaderHeight - ThemeConfig::SeparatorPadding);
            ...
            // Main Content
            ImGui::BeginChild("MainContent", ImVec2(ImGui::GetContentRegionAvail().x, 0), false);
