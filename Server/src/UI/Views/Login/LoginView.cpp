...
namespace UI {
    namespace Views {
        ...
        LoginView::LoginView(Application* app, Renderer* renderer)
            ...
            m_UsernameInput = std::make_shared<Components::InputText>("Kullanıcı Adı",
                                                                      &m_State->username);
            m_PasswordInput = std::make_shared<Components::InputText>(
                "Şifre",
                &m_State->password,
                m_State->showPassword
                    ? ImGuiInputTextFlags_EnterReturnsTrue
                    : ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_Password);
            m_ShowPasswordCheckbox = std::make_shared<Components::Checkbox>("Şifreyi Göster",
                                                                            &m_State->showPassword);
            ...
            m_PasswordInput->SetOnEnterPressed([this]() {
                if (m_App->GetAuthManager()->Login(m_State->username.c_str(),
                                                   m_State->password.c_str())) {
                    m_State->Reset();
                }
            });
        ...
        LoginViewState* LoginView::GetState() { return m_State.get(); }

        const char* LoginView::GetViewId() const { return GetStaticViewId(); }
