...
bool ManualMapDll(HANDLE hProc,
    ...
    // Mapping params
    BYTE* MappingDataAlloc = reinterpret_cast<BYTE*>(VirtualAllocEx(hProc,
                                                                    nullptr,
...
void __stdcall Shellcode(MANUAL_MAPPING_DATA* pData) {
    ...
    if (LocationDelta) {
        if (pOpt->DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC].Size) {
            ...
            while (pRelocData < pRelocEnd && pRelocData->SizeOfBlock) {
                UINT AmountOfEntries = (pRelocData->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION))
                                       / sizeof(WORD);
    ...
    if (pOpt->DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size) {
        ...
        while (pImportDescr->Name) {
            ...
            ULONG_PTR* pThunkRef = reinterpret_cast<ULONG_PTR*>(pBase
                                                                + pImportDescr->OriginalFirstThunk);
            ...
            for (; *pThunkRef; ++pThunkRef, ++pFuncRef) {
                if (IMAGE_SNAP_BY_ORDINAL(*pThunkRef)) {
                    *pFuncRef = (ULONG_PTR)
                        _GetProcAddress(hDll, reinterpret_cast<char*>(*pThunkRef & 0xFFFF));
                } else {
    ...
    if (pData->SEHSupport) {
        ...
        if (excep.Size) {
            if (!_RtlAddFunctionTable(reinterpret_cast<IMAGE_RUNTIME_FUNCTION_ENTRY*>(
